import { loadHttpKey, loadJsonFileKey } from './keys.js'
...
export async function atxSigner(config) {
  ...
  if (ENTITY_NAME == null || ENTITY_NAME == undefined) {
    ...
    if (PRIVATE_KEY_JSON) {
      ...
      if (isHttpUri(PRIVATE_KEY_URI)) {
        ...
        keyPair = await loadJsonFileKey(PRIVATE_KEY_URI)
      }
    }
    ...
    if (!PRIVATE_KEY_JSON) {
      ...
      if (isHttpUri(privateKeyUriUrl)) {
        ...
        keyPair = await loadJsonFileKey(privateKeyUriUrl)
      }
    }
  ...
  return {
     ...
     *
     * @param {*} credential
     * @param {object} plainObject the did object or a null if not provided (The public key)
     * @returns
     */
    sign: async (credential, plainObject=null) => {
      let signedData = await eddsaJxtSdk.sign(credential, keyPair, plainObject)
     ...
     * @param {object} plainObject the did object or a null if not provided (The public key)
     */
    verify: async (signedCredentialJxt, plainObject=null) => {
      let unpacked = await jsonxt.unpack(signedCredentialJxt, (resolver_name, template_uri) => {
        return template
      })

      let result = await eddsaJxtSdk.verify(unpacked, plainObject);
